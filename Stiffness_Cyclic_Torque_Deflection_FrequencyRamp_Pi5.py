...
def get_cycle_time_sec(cycle_index):
    return max(
        MIN_CYCLE_TIME_SEC,
        INITIAL_CYCLE_TIME_SEC - cycle_index * CYCLE_TIME_STEP_SEC,
    )


def get_deflection_rate_rad_per_sec(cycle_time_sec):
    return 4 * MAX_DEFLECTION_RAD / cycle_time_sec
...
def validate_test_config():
    ...
    if INITIAL_CYCLE_TIME_SEC <= 0:
        raise ValueError("INITIAL_CYCLE_TIME_SEC must be positive.")
    if CYCLE_TIME_STEP_SEC < 0:
        raise ValueError("CYCLE_TIME_STEP_SEC must be non-negative.")
    if MIN_CYCLE_TIME_SEC <= 0:
        raise ValueError("MIN_CYCLE_TIME_SEC must be positive.")
    if MIN_CYCLE_TIME_SEC > INITIAL_CYCLE_TIME_SEC:
        raise ValueError("MIN_CYCLE_TIME_SEC must not exceed INITIAL_CYCLE_TIME_SEC.")
    final_requested_cycle_time_sec = (
        INITIAL_CYCLE_TIME_SEC - (NUM_CYCLES - 1) * CYCLE_TIME_STEP_SEC
    )
    if final_requested_cycle_time_sec < MIN_CYCLE_TIME_SEC:
        raise ValueError(
            "Requested cycle-time schedule falls below MIN_CYCLE_TIME_SEC."
        )
...
if __name__ == "__main__":
    ...
    file_name = "Stiffness_FrequencyRamp_" + strftime("%y%m%d_%H%M%S")
    ...
    osl = OpenSourceLeg(
        tag="stiffness_frequency_ramp_pi5",
        actuators={"knee": knee},
        sensors={},
    )
    ...
    cycle_count = 0
    half_cycle_count = 0
    current_cycle_time_sec = get_cycle_time_sec(cycle_count)
    deflection_rate_rad_per_sec = get_deflection_rate_rad_per_sec(
    ...
    logger.track_function(
        [
            ...
            lambda: INITIAL_CYCLE_TIME_SEC,
            lambda: CYCLE_TIME_STEP_SEC,
            lambda: MIN_CYCLE_TIME_SEC,
            lambda: current_cycle_time_sec,
            lambda: deflection_rate_rad_per_sec,
        ...
        [
            ...
            "initial_cycle_time_sec",
            "cycle_time_step_sec",
            "min_cycle_time_sec",
            "current_cycle_time_sec",
            "deflection_rate_rad_per_sec",
            "hold_timeout_sec",
            "pos_command",
            "measured_deflection",
            "tau_futek",
    ...
    try:
        ...
        with osl:
            ...
            print(
                "Test in Progress: cycle schedule starts at {:.1f} s and "
                "decreases by {:.1f} s per cycle to {:.1f} s.".format(
                    INITIAL_CYCLE_TIME_SEC,
                    CYCLE_TIME_STEP_SEC,
                    MIN_CYCLE_TIME_SEC,
                )
            ...
            for t in clock:
                ...
                print(
                    ...
                    "hold_elapsed_s={:.2f}".format(
                        min(cycle_count + 1, NUM_CYCLES),
                        NUM_CYCLES,
                        current_cycle_time_sec,
                        np.rad2deg(deflection_rate_rad_per_sec),
                        knee.motor_current,
                ...
                if torque_abort_reached:
                    ...
                    if not test_complete:
                        ...
                        target_direction *= -1
                        print(
                            "Switching direction after reaching {:.4f} rad and {:.2f} Nm. "
                            "Active cycle T={:.1f} s, rate={:.2f} deg/s.".format(
                                measured_deflection,
                                tau_futek,
                                current_cycle_time_sec,
                                np.rad2deg(deflection_rate_rad_per_sec),
