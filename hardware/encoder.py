...
class AS5048A_Encoder:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rotations = 0
        self.rotations_comp = 0
        self.P = []

    def _start(self) -> None:
        print("Open encoder communication", self.__class__.__name__)
    ...
    def abs_ang(self) -> float:
        try:
            ...
            encAngRadOld = (from_twos_compliment(encAngleDataOld, 14) * 2 * np.pi) / AS5048A_Encoder.ENC_RESOLUTION
            encAngRadNew = (from_twos_compliment(encAngleDataNew, 14) * 2 * np.pi) / AS5048A_Encoder.ENC_RESOLUTION

            if encAngRadNew - encAngRadOld > np.pi: # check for big jumps
                self.rotations -= 1
            elif encAngRadNew - encAngRadOld < -np.pi: # check for big jumps
                self.rotations += 1

            return (encAngRadNew + (2 * np.pi)*self.rotations)   
        
    @property
    def abs_comp_ang(self) -> float:
        try:
            encAngleDataOld = AS5048A_Encoder._get_14bit(self._encdata_old[4:6])
            encAngleDataNew = AS5048A_Encoder._get_14bit(self._encdata_new[4:6])
            ...
            if encAngRadNew - encAngRadOld > np.pi: # check for big jumps
                self.rotations_comp -= 1
            elif encAngRadNew - encAngRadOld < -np.pi: # check for big jumps
                self.rotations_comp += 1

            return (np.polyval(self.P,encAngRadNew) + (2 * np.pi)*self.rotations_comp)  
...
if __name__ == "__main__":
    ...
    print(f"Zero registers:", knee_enc.zero_position)
    print(f"Enc output:", knee_enc.encoder_output)

    while True:
        try:
            knee_enc._update()
            print(knee_enc.position,knee_enc.abs_ang)
            # print(knee_enc.encoder_output)
