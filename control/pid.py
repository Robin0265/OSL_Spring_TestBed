...
class Smooth_Val_Plus_Deriv:
    """
        Moving Average Filter with adjustable window
    """
    
    def __init__(self, start_value, window, freq):
        self.vals = [start_value]
        # self.vals.append(start_value)
        self.window = window
        self.A = np.vstack([np.ones(window), np.arange(window)/freq]).T
...
class PID(object):
    ...
    def __init__(
        ...
        :param int_lim_scaled: Saturation specifically on the integral term as scalar based on 
            proportional contribution. Default is high so that it doesn't limit the integral 
            term much
        """
        ...
        self.integral_limits = (None,None)
        self.int_lim_scaled = int_lim_scaled
        ...
        self.SmoothError = Smooth_Val_Plus_Deriv(0.0,self.smooth_window,self.sample_freq) # (initial value, window)
        # self.SmoothInputError = Moving_Avg_Filter(0.0,3) # (initial value, window)
    ...
    def __call__(self, input_, dt=None):
        ...
        now = self.time_fn()
        if dt is None:
            dt = 1/self.sample_freq #now - self._last_time if (now - self._last_time) else 1e-16
        elif dt <= 0:
        ...
        error_smooth, d_error_smooth = self.SmoothError.update(error)
        
        d_input = input_ - (self._last_input if (self._last_input is not None) else input_)
        # d_input_smooth = self.SmoothInputError.update(d_input)
        d_error = error - (self._last_error if (self._last_error is not None) else error)
        # d_error_smooth = self.SmoothError.update(d_error)
        ...
        if not self.proportional_on_measurement:
            ...
            self._proportional = self.Kp * error_smooth
        else:
        ...
        self._integral += self.Ki * error_smooth * dt
        
        if self._proportional > 0:
            self.integral_limits = (-self.int_lim_scaled*self._proportional, self.int_lim_scaled*self._proportional)
        else:
            self.integral_limits = (self.int_lim_scaled*self._proportional, -self.int_lim_scaled*self._proportional)
    ...
    def __repr__(self):
        return (
            ...
            'integral_limits={self.integral_limits!r},'
